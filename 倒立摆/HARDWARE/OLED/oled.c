...
void OLED_Refresh_Gram(void)
{
	...
	{  
		...
		for(n=0;n<128;n++)OLED_WR_Byte(OLED_GRAM[n][i],OLED_DATA);    //在i列n个字节上显示数据
	}   
}
...
void OLED_ShowDecimals(u16 x,u16 y,const u8 *p,u8 size,int get)
{
     u8 test[10];
  while((*p<='~')&&(*p>=' '))	
	{
